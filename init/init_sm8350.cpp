...
void vendor_load_properties()
{
    ...
    std::string bl_model;
    if (bootloader.substr(0, MODEL_NAME_LEN_B2) == "G990B2") {
        bl_model = bootloader.substr(0, MODEL_NAME_LEN_B2);
    } else {
    ...
    if (device.size() == 0) {
        LOG(ERROR) << "Could not detect device, forcing t2q";
        device = "t2q";
    }
